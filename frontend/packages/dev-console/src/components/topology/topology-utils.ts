 ...
 */
export const getMonitoringStatus = (resource: OverviewItem): string => {
  const { events, obj } = resource;
  if (!['Deployment', 'DeploymentConfig'].includes(obj.kind)) return null;
  const eventStatus = _.filter(events[0], ['type', 'Warning']);
  return eventStatus.length ? 'Warning' : 'Normal';
};
...
export const createTopologyNodeData = (
  ...
  return {
    ...
    data: {
      ...
      monitoringStatus: getMonitoringStatus(dc),
    },
  };
};
